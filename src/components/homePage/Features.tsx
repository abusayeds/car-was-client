  ...
  return (
    <main>
      <p className=" text-xl md:text-4xl text-gray-700 pb-10 text-center uppercase font-titlefont ">
        OUR,s most popular servise
      </p>
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-8 items-center justify-center">
        {data?.data?.map((item: any) => (
          <div
            className="border md:h-[410px] md:overflow-y-auto flex flex-col gap-8 relative overflow-hidden hide-scrollbar group"
            key={item._id}
          >
            <img
            ...
            <div className="absolute inset-0 flex justify-center items-center bg-black bg-opacity-40 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100">
              <Link
                to="/servise-details"
